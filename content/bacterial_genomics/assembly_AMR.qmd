...
Because *de novo* assembly takes quite some time to run, we have already performed the assemblies for you. You can find the resulting FASTA files here:
...
Typicvally we perform QC on reads, but everythime we download or generate a bacterial genome assembly, it would be wise to check the quality. Often, when download from say RefSeq, you'll have quality info available. Here, we donsloaded the genomes from a Zenodo link for this coure, so we want to check it ourselves. 

::: {.callout-note}

Exercise 1: QC on the reference genomes
...
Good, we now have an idea of what we ....

### Finding featuires in the assemblies

Now that we have an idea of the quality of the assemblies, we can start looking for features in the genomes. We are particularly interested in antimicrobial resistance genes and plasmid replicon genes, but you can also look for other features if you like.

::: {.callout-note}

Exercise 2: Annotation, AMR genes, and rplicon genes
