...
Aside from GATK HaplotypeCaller, you might encounter, amongst others, the following tools:
...
The output of variant calling is a file in the Variant Call Format (`.vcf`), which we already encountered in the previous course's [section on vcf](https://pmoris.github.io/FiMAB-bioinformatics/content/unix/7-unix-redirection.html#sec-vcf).
...
Hard-filtering works by setting specific thresholds for one or multiple annotations (i.e. properties of a variant, like read coverage, proportion of forward/reverse reads, etc.) VQSR is a more powerful approach because it tries to combine all of these annotations to make a decision about good or bad variants, but to do this, it requires database of well-curated known variants as a training set. When these are available, this approach is highly recommended.
